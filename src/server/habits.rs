...
mod db_utils {
    ...
    pub async fn fetch_habits() -> Result<Vec<Habit>, ServerFnError> {
        with_db(|conn| {
            let mut stmt = conn
    ...
    pub async fn fetch_habit_by_id(id: Uuid) -> Result<Habit, ServerFnError> {
        with_db(|conn| {
            conn.query_row(
    ...
    pub async fn fetch_first_habit() -> Result<Option<Habit>, ServerFnError> {
        with_db(|conn| {
            conn.query_row(
    ...
    pub async fn create_new_habit(habit: Habit) -> Result<(), ServerFnError> {
        with_db(|conn| {
            conn.execute(
    ...
    pub async fn update_existing_habit(
        id: Uuid,
    ...
    pub async fn delete_habit_by_id(id: Uuid) -> Result<(), ServerFnError> {
        with_db(|conn| {
    ...
    pub async fn fetch_completed_days(id: Uuid) -> Result<Vec<NaiveDate>, ServerFnError> {
        with_db(|conn| {
            let mut stmt = conn
    ...
    pub async fn update_habit_start_date(
        id: Uuid,
...
// Server function exports remain unchanged
#[server]
pub async fn get_habits() -> Result<Vec<Habit>, ServerFnError> {
    db_utils::fetch_habits().await
}

#[server]
pub async fn get_habit(id: Uuid) -> Result<Habit, ServerFnError> {
    db_utils::fetch_habit_by_id(id).await
}

#[server]
pub async fn get_first_habit() -> Result<Option<Habit>, ServerFnError> {
    db_utils::fetch_first_habit().await
}

#[server]
pub async fn create_habit(habit: Habit) -> Result<(), ServerFnError> {
    db_utils::create_new_habit(habit).await
}

#[server]
pub async fn update_habit(id: Uuid, updates: UpdateHabitRequest) -> Result<(), ServerFnError> {
    db_utils::update_existing_habit(id, updates).await
}

#[server]
pub async fn delete_habit(id: Uuid) -> Result<(), ServerFnError> {
    db_utils::delete_habit_by_id(id).await
}

#[server]
pub async fn get_completed_days(id: Uuid) -> Result<Vec<NaiveDate>, ServerFnError> {
    db_utils::fetch_completed_days(id).await
}

#[server]
pub async fn save_completed_day(id: Uuid, date: NaiveDate) -> Result<(), ServerFnError> {
    db_utils::save_completed_day(id, date).await
}

#[server]
pub async fn delete_completed_day(id: Uuid, date: NaiveDate) -> Result<(), ServerFnError> {
    db_utils::delete_completed_day(id, date).await
}

#[server]
pub async fn update_habit_title(id: Uuid, title: String) -> Result<(), ServerFnError> {
    db_utils::update_habit_title(id, title).await
}

#[server]
pub async fn update_habit_color(id: Uuid, color: String) -> Result<(), ServerFnError> {
    db_utils::update_habit_color(id, color).await
}

#[server]
pub async fn update_habit_start_date(id: Uuid, start_date: NaiveDate) -> Result<(), ServerFnError> {
    db_utils::update_habit_start_date(id, start_date).await
}

#[server]
pub async fn update_habit_week_start(id: Uuid, week_start: String) -> Result<(), ServerFnError> {
    db_utils::update_habit_week_start(id, week_start).await
}